...
barcosPC:		.space 400	# Array com as informações de cada barco (1-Numero do Barco (tabuleiro) / 2-Tamnaho / 3-Contador (Verificar Afundou))
...
gerarPos:
	...
	#lw $a3, 16($sp)
	add $a3, $a3, $t1		# Meter $a3 na posição gerada aleatóriamente
	lw $a0, 4($sp)			# Receber o size do barco pela stack (pois perco o valor do $a0 noutras funções)
	#lw $a3, 16($sp)
	...
	lw $t4, -36($a3)		# recebe o valor da diagonal cima à direita de cima do tabulerio
	#bne $t4, '0', gerarPos		# if($t4 != '0') vai para gerarPos
	#bne $t4, 0, gerarPos
	bne $t4, '-', gerarPos
	...
	lw $t4, 44($a3)			# recebe o valor da diagonal baixo à direita do tabulerio
	#bne $t4, '0', gerarPos		# if($t4 != '0') vai para gerarPos
	#bne $t4, 0, gerarPos	
	bne $t4, '-', gerarPos
	...
	lw $t4, 36($a3)			# recebe o valor da diagonal baixo à esquerda do tabulerio
	#bne $t4, '0', gerarPos		# if($t4 != '0') vai para gerarPos
	#bne $t4, 0, gerarPos
	bne $t4, '-', gerarPos
	...
	lw $t4, -44($a3)		# recebe o valor da diagonal de cima à esquerda do tabulerio
	#bne $t4, '0', gerarPos		# if($t4 != '0') vai para gerarPos
	#bne $t4, 0, gerarPos	
	bne $t4, '-', gerarPos
	...
	forSizeBarco:
		...
		lw $t2, 0($a3)			# Recebe o valor dessa posição em $t2
...
forAddBarco:
	...
	#la $s0, tabuleiro
	lw $a3, 16($sp)
	add $a3, $a3, $t1			# Anda no tabuleiro para as várias posições
	sw $a1, 0($a3)				# Guarda o valor de $a1(letra do barco) na posição $a3(tabuleiro)
	addi $t9, $t9, 1			# I++
	j forAddBarco
addBarcoArrayBarcos:
	lw $a0, 4($sp)		# Receber da stack o valor de $a0 (size do Braco)
