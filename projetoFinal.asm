...
cicloMenu:
	...
	beq $t1, 'e', sairPrograma		# if ($t1 == 'e') -> sairPrograma
		...
		jogoSozinho:			
		jal fase1			# Chama a funcao fase1
		j cicloMenu			# Volta ao ciclo do menu
		jogoPc:
		jal fase2			# Chama a funcao fase2
		j cicloMenu			# Volta ao ciclo do menu
	sairPrograma:
...
add $a0, $s0, $0		# passa $s0 para $a0 como argumento
...
lw $s0, 4($sp)			# Recebe $s0 da stack
lw $s1, 8($sp)			# Recebe $s1 da stack
lw $s2, 12($sp)			# Recebe $s2 da stack
...
# $t2 -> tamanho do barco 
# $t4 -> pontuacoes		
# $s7 -> Editar tamanho de barcos ou nao (1 -> editar) / numero de barcos (menu utilizador)
...
# $s1 -> char coluna
# $s2 -> numero linha
# $s3 -> posi??o escolhida pelo utilizador final / size do barco acertado
# $s4 -> valores dos tabuleiros
# $s5 -> Valor a por no tabuleiro (X -> Bomba / 0 -> Agua)
# $s6 -> contadores de barcos
...
cicloJogo:
	...
	bne $s4, '-', jogadaRepetida	# if($s4 != '-') esta posicaoja foi inserida anteriormente 
		...
		syscall
		lw $a0, 32($sp)
		j sairJogo			# sai do jogo porque o jogador nao acertou nenhum barco
...
# $s2 -> char coluna
# $s3 -> numero linha
# $s4 -> posi??o escolhida pelo utilizador final / size do barco acertado
# $s5 -> valores dos tabuleiros
# $s6 -> Valor a por no tabuleiro (X -> Bomba / 0 -> Agua)
# $s7 -> contadores de barcos
...
cicloJogoPC:
	...
	bne $s5, '-', jogadaRepetidaUtilizador		# if($s5 != '-') esta posicaoja foi inserida anteriormente
		...
		cicloVerArrayBarcoUtilizador:		
			...
			#jal displayTabuleiroJogoPC			# para dar display ao tabuleiro na consola
	...
	beq $s5, 'X', jogadaRepetidaPC
		...
		cicloVerArrayBarcoPC:
			...
			la $s6, 'X'
			sw $s6, 0($s1)
			#jal displayTabuleiroJogo	# para dar display ao tabuleiro na consola
...
# $s0 -> Posicoes das Pecas do Barco
# $s1 -> posi??o do barco antes de chackar se vazia / posi??o do barco para add Numero
# $s2 -> Valor na Pos do tabuleiro
# $s3 -> Axis (Horizontal = 0 / Vertical = 1)
...
# $s4 -> size Barco *4
# $s5 -> Saber se vou validar cima/baixo 	(0 -> validar ambas / 1 -> nao validar cima / 2 -> n?o validar baixo)
# $s6 -> Saber se vou validar esquerda/direita 	(0 -> validar / 1 -> nao validar esquerda / 2 -> n?o validar direita)
# $s7 -> I
